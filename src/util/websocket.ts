...
export class Connection {
  ...
  send(message: object | string) {
    ...
    if (this.server) {
      ...
      console.log(
        "Sending failed: No WebSocket connection, trying to connect again..."
      );
      this.connect();
    }
  }
  ...
  async listen(callback: (message: string) => void) {
    if (this.server) {
      ...
      console.log(
        "Listening failed: No WebSocket connection, trying to connect..."
      );
      await this.connect();
      this.listen(callback);
    }
  ...
  searchForServer(searchTarget: string): Promise<any> {
