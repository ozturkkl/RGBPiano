<script lang="ts">
  ...
  const activeTab = Store.getPersistent<ActiveTab>(
    'activeTab',
    'led-visualizer',
    (v) => v !== 'settings',
  )
...
<div class="App drawer">
  ...
  <div class="drawer-content flex flex-col">
    <div
      class="navbar bg-base-300 w-screen h-11 min-h-11 p-0 flex z-10"
      class:app-drag={!sidebarToggled}
    >
      <div class="flex-none sm:hidden">
        <label
          for="app-drawer"
          aria-label="open sidebar"
          class="btn btn-square rounded-none btn-ghost"
        >
          <svg
    ...
    <div
      class="absolute w-screen h-screen max-h-screen pt-11 overflow-hidden *:overflow-auto *:h-full *:p-3 *:sm:p-8"
    >
      <!-- Page content here -->
  ...
  <div class="drawer-side">
    ...
    <ul class="menu bg-base-200 h-screen pt-14 p-4 max-w-screen">
      ...
      <button
        ...
        class:text-primary={$activeTab === 'led-visualizer'}
        class:hover:bg-transparent={$activeTab === 'led-visualizer'}
        on:click={() => (($activeTab = 'led-visualizer'), (sidebarToggled = false))}
        >LED Visualizer</button
      >
    </ul>
  </div>
</div>
